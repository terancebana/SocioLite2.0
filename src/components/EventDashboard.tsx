...
export default function EventDashboard({ userId }: { userId: string }) {
      ...
      <div className="flex gap-1 p-1 bg-surface-tertiary rounded-lg">
        {[
          ...
          <button key={k} onClick={() => setFilter(k as typeof filter)}
            className={`flex-1 px-2 py-1.5 text-xs font-medium rounded-md transition-all ${filter === k ? "bg-white text-ink-primary shadow-soft" : "text-ink-secondary hover:text-ink-primary"}`}>
            {l} <span className="opacity-40 ml-0.5">{n}</span>
          </button>
        ))}
      </div>

      <div className="space-y-1.5 max-h-[calc(100vh-260px)] lg:max-h-[calc(100vh-220px)] overflow-y-auto pr-0.5">
        {sorted.length === 0 ? (
          <div className="text-center py-10">
            <div className="w-10 h-10 mx-auto mb-2 rounded-full bg-surface-tertiary flex items-center justify-center">
              <svg className="w-5 h-5 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <p className="text-sm text-ink-secondary">No events</p>
            {searchTerm && <p className="text-xs text-ink-muted mt-0.5">Try a different search</p>}
          </div>
        ) : (
          sorted.map((event) => (
            <button key={event.id} onClick={() => selectEvent(event)}
              className={`w-full text-left p-3.5 rounded-xl border transition-all ${
                selectedEvent?.id === event.id ? "border-brand-300 bg-brand-50/50 shadow-sm" : "border-transparent bg-white hover:bg-surface-secondary hover:border-slate-200"
              }`}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <h3 className="text-sm font-semibold text-ink-primary truncate">{event.title}</h3>
                  <p className="text-2xs text-ink-muted mt-0.5">{format(new Date(event.event_date), "MMM d · h:mm a")}</p>
                </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white shrink-0">
        <button onClick={() => { setMobileView("list"); setSelectedEvent(null) }}
          className="lg:hidden btn-ghost btn-icon -ml-2 shrink-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold text-ink-primary truncate">{selectedEvent.title}</h2>
            {selectedEvent.creator_id === userId && (
              <button onClick={() => setIsRenameModalOpen(true)} className="btn-ghost btn-icon shrink-0 text-ink-muted hover:text-brand-500">
          ...
          <div className="flex items-center gap-2 text-xs text-ink-muted">
            <button onClick={() => setIsParticipantListOpen(true)} className="hover:text-ink-secondary transition-colors">
  ...
  const emptyState = (
    <div className="flex flex-col items-center justify-center h-full text-center py-12 px-4">
      <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-3xl bg-surface-tertiary flex items-center justify-center mb-4 sm:mb-6">
        <svg className="w-8 h-8 sm:w-10 sm:h-10 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
            d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
        </svg>
      </div>
      <h3 className="text-base sm:text-lg font-semibold text-ink-primary mb-1">Select an event</h3>
      <p className="text-sm text-ink-secondary max-w-xs">Choose an event from the list or create a new one to start chatting.</p>
  ...
  return (
    <>
      ...
      <div className="hidden lg:grid lg:grid-cols-3 gap-0 h-full">
        <div className="lg:col-span-1 overflow-hidden flex flex-col border-r border-slate-200/60 bg-white">
          <div className="p-4 overflow-y-auto flex-1">{sidebar}</div>
        </div>
        <div className="lg:col-span-2 overflow-hidden flex flex-col">
          {selectedEvent ? chatView : (
            <div className="flex-1 flex items-center justify-center bg-surface-secondary">{emptyState}</div>
          )}
        </div>
      </div>
      ...
      <div className="lg:hidden flex flex-col h-[calc(100dvh-140px)]">
        ...
        {mobileView === "chat" && selectedEvent && (
          <div className="flex-1 flex flex-col min-h-0">{chatView}</div>
        )}
      </div>

      {/* === BOTTOM BAR (mobile only) === */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg border-t border-slate-200/60 safe-bottom z-40">
        <div className="flex items-center justify-around h-14 px-2">
          <button onClick={() => { setMobileView("list"); setSelectedEvent(null) }}
            className={`flex flex-col items-center justify-center gap-0.5 px-4 py-1 rounded-xl transition-colors ${mobileView === "list" ? "text-brand-600" : "text-ink-muted"}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button onClick={() => selectedEvent ? setMobileView("chat") : null}
            className={`flex flex-col items-center justify-center gap-0.5 px-4 py-1 rounded-xl transition-colors ${mobileView === "chat" ? "text-brand-600" : "text-ink-muted"}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
                d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
